...
function Client(accessKey, businessId){
   ...
   * @return {Promise}          [description]
   */
  this.downloadRawDocument = function(document, stream) {
    return downloadRawDocument(document, stream, 0, config.DOCUMENT_RAW_URL);
  };

  function downloadRawDocument(document, stream, auditTrail = 0, type = config.DOCUMENT_FINAL_URL) {
    return new Promise(function(resolve, reject) {
      getBusinessId().then(function (businessId) {
        if (!document) {
