...
function Client(accessKey, businessId){
   ...
   * @return {Promise}                     [description]
   */
  this.downloadFinalDocument = function (document, stream, auditTrail = false) {
    if(!document.getIsCompleted())
      throw new Error('To Download the final File the Document needs to be completed first');
    return downloadDocument(document, stream, auditTrail);
  };
  ...
  /**
   * Downloads the raw Document to a stream
   * Returns true if saving was successful, otherwise false
   * @param  {Document} Document [description]
   * @param  {Any} stream    [description]
   * @return {Promise}          [description]
   */
  this.downloadDocument = function(document, stream) {
    return downloadDocument(document, stream, 0, config.DOCUMENT_RAW_URL);
  };

  function downloadDocument(document, stream, auditTrail = 0, type = config.DOCUMENT_FINAL_URL) {
    return new Promise(function(resolve, reject) {
      getBusinessId().then(function (businessId) {
        if (!document) {
        ...
        return request.startDownload(stream).then(resolve).catch(reject);
      }).catch(function(err) {
        console.error(err)
      });
    });
